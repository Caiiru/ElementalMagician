...
public class EnemyAi : MonoBehaviour
{
    ...
    private void PathFollow()
    {
        ...
        isGrounded = Physics2D.Raycast(startOffset, -Vector3.up, jumpCheckDistance,floorMask);
        if(isGrounded)
            Debug.DrawRay(transform.position, -Vector3.up * jumpCheckDistance, Color.green);
        else
        ...
        if (jumpEnabled && isGrounded && !isInAir && !isOnCoolDown)
        {
            if (direction.y > jumpNodeHeightRequirement)
            {
                if (isInAir) return; 
                isJumping = true;
                rb.velocity = new Vector2(rb.velocity.x, enemyStats.JumpForce);
